...
<body>
    ...
    <main>
        ...
        if (!$DB) {
            ...
            $getAllLivreRequest = "SELECT * FROM livres";

            $getAllLivre = mysqli_query($DB, $getAllLivreRequest);

            $allLivres = mysqli_fetch_all($getAllLivre, MYSQLI_ASSOC);
            ...
            foreach ($allLivres as $livre) {
                ...
                foreach ($livre as $livreProperty => $livrePropertyValue) {
                    if ($livreProperty != "Id_Livre") {
                        $livreLine .= "<td data-cellTitle= '$livreProperty'>" . $livrePropertyValue . "</td>";
                    }
                }
